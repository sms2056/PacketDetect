...
class TcpPack:
    ...
    def __init__(self, source, source_port, dest, dest_port, flags, seq, ack_seq, body):
        ...
        self.ack = (flags >> 4) & 1
        # urg = (flags >> 5) & 1
        self.payload_len = len(self.body)
...
class UdpPack:
    """ a udp packet, header fields and data. """

    def __init__(self, source, source_port, dest, dest_port, length, checksum, body):
        self.source = source
        self.source_port = source_port
        self.dest = dest
        self.dest_port = dest_port
    ...
    def __str__(self):
        return "%s:%d  -->  %s:%d, length:%d, checksum:%s size:%d" % \
               (self.source, self.source_port, self.dest, self.dest_port, self.length,
                self.checksum, self.payload_len)
...
def parse_udp_packet(udp_packet):
    ...
    source_port, dest_port, length, checksum = struct.unpack(b'!HHHH', udp_header)

    body = udp_packet[udp_base_header_len:]
    return source_port, dest_port, length, checksum, body
...
def read_tcp_packet(read_packet,count = 0):
    num = 0
...
def read_udp_packet(read_packet,count = 0):
    num = 0
    for link_type, micro_second, link_packet in read_packet():
        link_layer_parser = get_link_layer_parser(link_type)
        network_protocol, link_layer_body = link_layer_parser(link_packet)
        transport_protocol, source, dest, ip_body = parse_ip_packet(network_protocol, link_layer_body)
        # not tcp, skip.
        if transport_protocol == TransferProtocol.UDP:
            source_port, dest_port, length, checksum ,body = parse_udp_packet(ip_body)
            yield UdpPack(source, source_port, dest, dest_port, length, checksum ,body)
        if count > 0:
            num = num + 1
            if num == count:
                break

def Read_Packet(read_packet,count = 0):
    num = 0
    for link_type, micro_second, link_packet in read_packet():
        link_layer_parser = get_link_layer_parser(link_type)
        network_protocol, link_layer_body = link_layer_parser(link_packet)
